...
function greaseCiphersuites(greaseConfig: GreaseConfig): CiphersuiteName[] {
  return grease(greaseConfig).map((n) => n.toString() as CiphersuiteName)
}

function greaseCredentials(greaseConfig: GreaseConfig): CredentialTypeName[] {
  return grease(greaseConfig).map((n) => n.toString() as CredentialTypeName)
}

function greaseExtensions(greaseConfig: GreaseConfig): Extension[] {
