...
/** Splits `sig(64) ‖ seq(8, big-endian) ‖ v` — the shape src/did/dht/packet.ts
...
// A miss costs the same full Kademlia traversal as a hit — measured at 3-6.5s
// through this gateway — and, unlike a hit, nothing remembered it: the read
// cache above stores payloads, and a miss has none. So every client resolve of
// a DID the DHT can't answer for (a contact who never registered a device, a
// record that hasn't propagated yet, a typo) paid full price, every time,
// from every client.
//
// Kept far shorter than the hit TTL, and dropped the instant a PUT lands for
// that key: "not published yet" is precisely the state a brand-new identity is
// in moments before it publishes, and answering 404 to its owner's own first
// lookup would be a far worse bug than the latency this fixes.
const MISS_CACHE_TTL_MS = 5_000
...
export class PkarrGateway {
  ...
  private missCache = new Map<string, number>() // hex pubkey → when the DHT last had nothing
  ...
  async get(pubkey: Buffer): Promise<Buffer | null> {
    ...
    const missedAt = this.missCache.get(hex)
    if (missedAt !== undefined) {
      if (Date.now() - missedAt < MISS_CACHE_TTL_MS) return null
      this.missCache.delete(hex)
    }
    ...
    if (!res?.v) { this.rememberMiss(hex); return null }
  ...
  private rememberRead(hex: string, payload: Buffer): void {
    this.missCache.delete(hex) // there is a record after all
  ...
  /** Bounded the same way as readCache, and by the same insertion-order trick. */
  private rememberMiss(hex: string): void {
    this.missCache.delete(hex)
    this.missCache.set(hex, Date.now())
    if (this.missCache.size > READ_CACHE_MAX) {
