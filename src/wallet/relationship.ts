  ...
  return {
    async ensureContact(counterpartyDid): Promise<ContactKeyV1> {
      ...
      if (!pending) {
        ...
        pending = { result: initiated, promise, resolve }
      ...
      return Promise.race([pending.promise, timeout]).finally(() => {
        ...
        if (pendingByCounterparty.get(counterpartyDid) === pending) pendingByCounterparty.delete(counterpartyDid)
        if (pendingByOwnKid.get(pending.result.pending.peer.xKid) === pending) pendingByOwnKid.delete(pending.result.pending.peer.xKid)
      })
    },
