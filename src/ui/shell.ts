...
import { setActiveConversation } from '../push/client.ts'
import { inboxToHash } from '../utils.ts'
import type { ProcessedMessage } from '../state.ts'
import type { AccountSession } from '../types.ts'
// Circular:
import { render, addMessage, syncDockPosition, keepAtBottom } from './thread.ts'
...
const MOBILE_SHOW_LEFT_KEY = 'biset-mobile-show-left'
{
  ...
  if ($app) {
    new MutationObserver(() => {
      const showLeft = $app.classList.contains('show-left')
      try { localStorage.setItem(MOBILE_SHOW_LEFT_KEY, showLeft ? '1' : '0') } catch {}
      // Landing back on the inbox list should always show its header chrome
      // (search field, toggle, hamburger) regardless of scroll position —
      // same reasoning as scrolling to the top.
      if (showLeft) {
        for (const id of ['lp-search-wrap', 'main-toggle', 'lp-hamburger-left']) {
          document.getElementById(id)?.classList.remove('lp-search-hidden')
        }
      }
...
export function addPendingMessage(
  ...
  renderedKeys.add(messageKey(processed.msg))
  return tempId
}
...
export function removePendingMessage(tempId: string) {
  ...
  renderedKeys.delete(messageKey(removed.msg))
}
...
export async function sendReply(
  ...
  const restoreBottom = keepAtBottom('instant')
  ...
  const { ok, error, partial } = await jmapCreateEmail(
    toAddrs, body, replySubject, inReplyTo, groupOpts, references,
    currentInbox?.user, relayForSend(currentInbox?.contact ?? ''), attachments,
  )
  if (ok) {
    // A DIDComm send fans out to every device the recipient has registered,
    // and a device that couldn't be reached simply never gets the message —
    // there is no retry and no server-side copy for it to catch up from. That
    // used to be a console warning nobody has open. Say it out loud instead:
    // the message went, but not everywhere it was meant to.
    if (partial) showSysMsg(`Sent, but only reached ${partial.delivered} of ${partial.total} of their devices`, 4000)
    await fetchMessages()
    loadLeftInboxes()
  } else {
...
export async function fetchMessages() {
  ...
  // Messages that weren't already known before this fetch. hasIncoming above
  // is true for a conversation's entire existing history, so this narrower
  // test is what "actually new mail" means here. wasFirstLoad is excluded
  // outright: that's the initial load of whichever conversation was just
  // opened, never "new" mail.
  //
  // No notification is raised from here. This path only ever saw the
  // conversation the user already has open — so it notified about the very
  // thread on screen, and then marked it read one line later — and it used
  // `new Notification()`, which an installed iOS PWA does not display at all.
...
export function startPolling() {
  ...
  if (!_visibilityHandlerInstalled) {
    ...
    document.addEventListener('visibilitychange', () => {
      // Hidden means the conversation on screen is no longer "being read", so
      // the Service Worker should stop suppressing notifications for it.
      if (document.visibilityState !== 'visible') { setActiveConversation(null); return }
      if (currentInbox && !inMenuMode()) setActiveConversation(inboxToHash(currentInbox))
      if (!sessions.length) return
      startSync(sessions).then(() => { fetchMessages(); loadLeftInboxes() })
    })
  }
