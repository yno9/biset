...
// no home in this rewrite yet (no at-rest protection, PLAN.md §7 scope) —
// dropped rather than ported half-working. showMnemonicOnce/promptForMnemonic
// DO have a home again as of identity/webvh/prerotation.ts: pre-rotation's
// Sign/Spare Key phrases need exactly this display/input pair.
import { seedToMnemonic, isValidMnemonic, mnemonicToSeed } from '../identity/seed.ts'
...
/** A freshly generated (not the identity's root) recovery phrase — the
 * pre-rotation Sign/Spare key, shown once right after it's generated.
 * Resolves when the user confirms they saved it (or closes the box, which
 * counts the same way `renderPhrase`'s own "I've saved it" button always
 * has — there is no verification step here, same reasoning as showMnemonic
 * itself). */
 ...
 * block, validates it as a real BIP39 mnemonic before resolving. Used
 * wherever a saved pre-rotation phrase needs to be produced (rotating into
 * it, or deactivating pre-rotation — both require it, see prerotation.ts's
 * own header on why). Resolves `null` on cancel. */
export function promptForMnemonic(opts: { title: string; subtitle: string; badges?: string[]; expectedFingerprint?: string; expectedHashes?: string[] }): Promise<string | null> {
  return new Promise(resolve => {
    ...
    // Same chip convention as renderPhrase — names WHAT this prompt is
    // asking for, since "your recovery phrase" is ambiguous once an
    // identity has both a root phrase and a sign-key phrase.
    for (const b of opts.badges ?? []) {
      const badge = document.createElement('span')
      badge.textContent = b
      badge.style.cssText = 'font-family:ui-monospace,monospace;font-size:11px;font-weight:800;letter-spacing:0.04em;color:var(--accent);background:var(--input-bg);border-radius:5px;padding:2px 6px;flex-shrink:0'
    ...
    // Which key the typed words actually are, echoed live, so a paper
    // labelled at display time (renderPhrase's own fingerprint line) can be
    // matched BEFORE submitting — Root / Sign / Spare can all be in play at
    // once and "wrong words" after the fact is a poor way to find out.
    ...
    const refreshEcho = () => {
      ...
      // outright (it is in the log's updateKeys). A Spare Key prompt only
      // has hash(multikey) — nextKeyHashes never carries the key itself,
      // that being the whole point of a commitment — so it checks in hash
      // space while still echoing the multikey, which is what the paper
      // was labelled with at display time.
