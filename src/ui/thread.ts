...
export async function addMessage(msg: ProcessedMessage['msg']): Promise<boolean> {
  ...
  if (renderedKeys.has(key)) {
    const existing = processedMessages.find(p => messageKey(p.msg) === key)
  ...
  // If this incoming message matches a pending stub (same from), drop the stub.
  {
    ...
    if (dropIdx >= 0) {
      ...
      renderedKeys.delete(messageKey(p.msg))
    }
  }
...
export function makeThreadCard(group: ThreadGroup, focused: boolean) {
  ...
  const renderVisible = (progressive: boolean) => {
    ...
    const renderChunk = () => {
      ...
      // Older messages go in ABOVE, which pushes everything down; a reader
      // sitting at the end wants to stay there. Same snapshot-then-restore as
      // the two append-at-the-bottom sites, so the threshold and the notion of
      // "the end" are shared rather than re-guessed here (this used to carry
      // its own 20px copy, which was narrower than scrollToFocused's own
      // resting slack and so misfired on every open — 2026-07-29).
      const restore = keepAtBottom('instant')
  ...
  if (focused) {
    ...
    // Bare verbs, no trailing address: who you're replying to (and which
    // identity you'd send a new message from) is already on screen — the
    // conversation header and the send button's own avatar — so repeating it
    // inside the field only made the placeholder long enough to clip.
    const replyPlaceholder = 'Reply'
    const composePlaceholder = 'New message'
...
/** How far a conversation may be scrolled before its own content starts
 * leaving the top: the height of the other threads listed above it, which are
 * meant to scroll away. Everything that must NOT scroll away — the installed
 * PWA's Dynamic Island reserve — lives outside this scroll container entirely
 * (style.css section 27), so there is nothing to subtract here and this is the
 * same number in the browser and in the PWA. */
...
/** Breathing room the last message keeps above the reply dock, when it can be
 * afforded. See updateScrollSpacer for when that is. */
const THREAD_BOTTOM_GAP = 14

/** Sizes #scroll-spacer, which is the ONLY source of space below the last
 * message. Everything here scrolls to the end of the range, so whatever this
 * leaves at the bottom is exactly what shows above the dock — the gap is a
 * consequence of this number, not a separate mechanism anyone has to align
 * with (an unconditional padding elsewhere in the content is what made "rest
 * above the dock" and "scroll to the end" two different positions that fought
 * each frame, 2026-07-29).
 *
 * Two cases, and the split is the point:
 *
 *  - The conversation FITS: pin the end at topFloor(), so opening it can't
 *    scroll its own header (or the notch strip) out of sight. Whatever space
 *    is left over becomes the gap for free — usually far more than
 *    THREAD_BOTTOM_GAP, which is just the empty room below a short thread.
 *  - It OVERFLOWS: the top is scrolled away regardless, so the gap costs
 *    nothing that was visible anyway — add it.
 *
 * That asymmetry is deliberate: a fixed gap in the overflowing case is fine,
 * but the same gap in the fitting case would push the conversation's own top
 * off screen to make room for empty space, which is what a conversation
 * opening half-way up looked like (2026-08-02, user-reported).
 *
 * Measured rather than derived: zeroing the spacer and reading the real
 * overflow back can't drift from the layout, and this stops needing to know
 * which elements exist. The version that summed title + conv-meta +
 * active-thread + dock silently omitted #past-threads' own height. */
...
// How far from the end still counts as "the reader is at the bottom".
...
export function scrollToBottomIfNear() {
  ...
  if (!outer) return
  updateScrollSpacer()
  if (outer.scrollHeight - outer.scrollTop - outer.clientHeight < NEAR_BOTTOM) scrollToEnd(outer)
}

/** Snapshot whether the reader is at the bottom BEFORE appending, and put
 * them back there after. The measurement has to happen first — the appended
 * message is itself what moves the end away, so checking afterwards would
 * always read as "scrolled up" and never follow. Both places that append to
 * the end of a live conversation (shell.ts after sending, appendMsgToDOM on
 * arrival) carried their own copy of this before/after pair; this is that
 * pair, once. */
export function keepAtBottom(behavior: ScrollBehavior = 'smooth'): () => void {
  const outer = document.getElementById('outer')
  const wasNear = !!outer && outer.scrollHeight - outer.scrollTop - outer.clientHeight < NEAR_BOTTOM
  return () => { if (outer && wasNear) scrollToEnd(outer, behavior) }
}

// The one place that answers "where is the bottom": the end of the range,
// always. The breathing room above the dock is .t-messages' own bottom
// padding (style.css) — part of the content, so it is already inside that
// answer and no scrolling code has to know it exists. The guard skips
// conversations too short to scroll (all padding, no overflow) — scrolling
// those does nothing but can still fire a spurious smooth-scroll animation.
function scrollToEnd(outer: HTMLElement, behavior: ScrollBehavior = 'smooth') {
  const padB = parseFloat(outer.style.paddingBottom) || 0
  if (outer.scrollHeight - padB <= outer.clientHeight + 1) return
...
export function scrollToFocused(smooth = false) {
  const doScroll = () => {
    ...
    const outer = document.getElementById('outer')
    if (!outer) return
    // The common case is just "the end of the scroll range" — no geometry, no
    // dock-relative arithmetic. Two facts make that sufficient: the breathing
    // room above the dock is .t-messages' own bottom padding, so the end
    // already IS the last message resting that far above the dock; and
    // updateScrollSpacer has just guaranteed the end never goes past
    // topFloor(), so a conversation shorter than the viewport cannot scroll
    // its own top away. Every snap-to-end path (renderChunk's pin,
    // scrollToBottomIfNear, the ↓ button) lands here too, by construction
    // rather than by agreement — which is what the old "stop just short of
    // the end" target could never do.
    let target = outer.scrollHeight - outer.clientHeight

    // One exception, which does need geometry: a last message TALLER than the
    // usable strip can't be shown whole either way, so pin its top just below
    // the sticky header rather than chase its tail (which would hide the
    // header). #thread-title-row is the only position:sticky element that
    // overlays scrolled-under content, and it covers stickyTop + titleH — in
    // the standalone PWA it sticks at top:env(safe-area-inset-top), with the
    // Dynamic Island strip above it; both are 0 in the browser. (#conv-meta
    // scrolls away normally and must NOT be added, or this under-scrolls.)
    ...
    if (last) {
      ...
      const padB = parseFloat(outer.style.paddingBottom) || 0
      if (last.offsetHeight > outer.clientHeight - padB - viewTop) {
...
export function render(smooth = false, keepScroll = false) {
  ...
  if (!groups.length) {
    // Header state has to be set HERE too, not only on the has-messages path
    // below — this branch returns early, so whatever title was in the header
    // survives. Opening an empty conversation straight from a menu page left
    // its title ("account", "config", …) sitting above the empty thread
    // (2026-07-28, user-reported, seen alongside the DIDComm filtering bug in
    // did/contacts.ts's addressesForContactKey). Same values the non-empty
    // path uses for a subject-less thread, so the two agree.
    ...
    const $emptyGroupIcon = document.getElementById('header-group-icon')
    if ($emptyGroupIcon) $emptyGroupIcon.style.display = 'none'
  ...
  if ($convTo) {
    ...
    if (didBadge) {
      ...
      // identifier anyone can paste anywhere (PLANWEBVH.md §3.1's
      // internal-vs-wire boundary). currentDidForKey is the reverse lookup;
