...
export function encodeMultikey(ed25519PublicKey: Uint8Array): string {
  if (ed25519PublicKey.length !== KEY_LEN) throw new Error(`encodeMultikey: expected ${KEY_LEN}-byte public key`)
  const prefixed = new Uint8Array(2 + KEY_LEN)
  prefixed[0] = ED25519_PUB_MULTICODEC[0]
  prefixed[1] = ED25519_PUB_MULTICODEC[1]
  prefixed.set(ed25519PublicKey, 2)
