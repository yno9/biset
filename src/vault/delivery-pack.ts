...
function eventKind(value: unknown): value is VaultEventV1['kind'] {
  return value === 'message.add' || value === 'message.edit' || value === 'message.tombstone' || value === 'mailbox.set' || value === 'keyword.set' || value === 'thread.set' || value === 'reaction.set' || value === 'read.set' || value === 'settings.set' || value === 'transport.result' || value === 'contact-key.set' || value === 'credential.openpgp.set'
}
