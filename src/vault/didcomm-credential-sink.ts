...
import { buildDidCommPrivateCredential, type DidCommCredentialBuildContext, type DidCommPrivateCredentialV1 } from './didcomm-credential.ts'
...
export interface DidCommCredentialVaultSinkOptions {
...
export interface DidCommCredentialStoreResult {
...
export class DidCommCredentialVaultSink {
  constructor(private readonly options: DidCommCredentialVaultSinkOptions) {
    if (!options.identityId || !options.actorDeviceId) throw new TypeError('DIDComm credential sink identity is required')
  }
  ...
  async store(credential: DidCommPrivateCredentialV1): Promise<DidCommCredentialStoreResult> {
    ...
    assertActiveVaultSegment(this.options.identityId, segment, 'DIDComm credential')
    const record = await buildDidCommPrivateCredential(credential, {
    ...
    } satisfies DidCommCredentialBuildContext, this.options.signer)
