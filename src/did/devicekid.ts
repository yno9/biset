...
//   - **Routing 2.0** — a Forward names its target by kid. It is the only
//     addressing information a mediator, which cannot decrypt anything, has.
//   - **The mediator's own bookkeeping** — one queue per kid, and the keylist
//     that authorizes delivery to it.
//
...
// tombstones, `removedKeyNs`), and a mistake is unrecoverable rather than
// merely wrong. ARC.md records the failure this produced: a sole device that
// logged out and restored got `#k1` again — a genuinely different key under an
// identifier the mediator had cached for ten minutes, so every authcrypt to it
// failed with "integrity check failed" until the cache expired.
//
...
// produce the same one. `resolveConfirmedAbsent`'s "refuse to guess a slot
// while the network is down" guard, and the tombstones' second job, both stop
...
// the document still binds key to identity, so a collision would only mean two
// devices claiming one name, at a cost of ~2^64 work to arrange.
//
// ## Legacy
//
...
// keep being able to address one. `isLegacyKid` tells them apart, which is
// what the rename migration keys off.
 ...
 * again; they are read, and migrated away from (didcomm-devices.ts's rename). */
...
/** The legacy slot number, for the one caller that still needs it: did:dht's
 * wire format names its verification-method records `_k<n>._did` and lists
 * `vm=k<n>`, which is numeric by that spec's own example. Undefined for a
 * derived kid, which that encoder handles by using the suffix as the label. */
export function legacySlotOf(kidOrFragment: string): number | undefined {
  const m = /^#k(\d+)$/.exec(fragmentOf(kidOrFragment))
