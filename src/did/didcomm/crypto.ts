 ...
 * today's ciphertext (HNDL, PLAN.md §1) even if X25519 alone is eventually
 * broken. Order (Ze, Zs, then Zpq) is arbitrary but fixed, matching how
 * deriveEcdh1PU orders Ze before Zs. */
...
function buildProtectedHeader(
  ...
  const header: Record<string, unknown> = {
    ...
    // PLAN.md "did:webvh PQハイブリッド化", Phase 2: biset-specific field, not
    // part of DIDComm v2 — carries the ML-KEM-768 encapsulation this JWE also
    // derives its CEK-wrapping key from. Only ever produced/consumed between
    // two biset peers that negotiated a hybrid `alg* (both published a
    // `#kk<n>` keyAgreement entry); a non-hybrid recipient never sees this
    // field, since the sender only reaches for the hybrid `pack*` functions
...
/** Hybrid authcrypt (PLAN.md "did:webvh PQハイブリッド化", Phase 2): same
 * ECDH-1PU/A256CBC-HS512 shape as packAuthcrypt, plus an ML-KEM-768
 * encapsulation folded into the key-wrapping key (deriveEcdh1PUHybrid) and
 * carried in the protected header's `pqKem` field so the recipient can
 * decapsulate. biset-specific `alg` — never sent to a peer that hasn't
 * itself published a `#kk<n>` ML-KEM-768 keyAgreement entry (the caller's own
 * negotiation, not this function's concern: this function assumes the
 * caller already decided hybrid is safe to use). */
 ...
 *
 * `fresh` asks for a genuinely re-resolved key rather than a cached one. Only
 * pickup.ts sets it, and only on a second attempt after unpacking failed: a
 * resolver that caches (the browser's does, across reloads — didcomm/
 * sender-keys.ts) would otherwise turn one wrong stored key into a message
 * that can never be opened. A resolver that doesn't cache can ignore it. */
...
/** Dispatches to unpackAuthcryptHybrid or plain unpackAuthcrypt by reading the
 * JWE's own `alg` — the receiving side's half of the negotiation PLAN.md
 * "did:webvh PQハイブリッド化" Phase 2 describes (the SENDING side decides via
 * mlkemPublicKeyOf, message.ts). A device without its own ML-KEM-768 key
 * (`self.mlkemPrivateKey` unset — an older device, or did:dht which never
 * gets one) can never legitimately receive a hybrid JWE addressed to it, so
 * that combination throws rather than silently mishandling it. */
