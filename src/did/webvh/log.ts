// DID Log entry construction/validation and JSONL codec (DIDWEBVHFEAT.md §4-5,
...
export interface LogParameters {
  ...
  portable?: boolean
  witness?: Record<string, unknown> // always {} — witness unimplemented (PLANWEBVH.md §2/§7)
  watchers?: string[]
  deactivated?: boolean
  ttl?: number
}
...
export function entryHashOf(versionId: string): string {
  const m = /^\d+-(.+)$/.exec(versionId)
 ...
 * full previous entry's versionId otherwise (DIDWEBVHFEAT.md §4, "Entry Hash
 * Generation": "the predecessor versionId is the SCID (itself a hash)" for
 * entry 1). */
...
/** Assembles a new entry's final versionId ("N-entryHash"). */
export function buildVersionId(n: number, predecessorVersionId: string, versionTime: string, parameters: LogParameters, state: object): string {
  return `${n}-${generateEntryHash(predecessorVersionId, versionTime, parameters, state)}`
}
 ...
 * OMITTED here, not restated — publish.ts used to hand resolveParameters'
 * full output straight to the log, which re-asserted every field on every
 * single update. Mostly harmless UNTIL `portable`: did:webvh v1.0 makes
 * restating `portable: true` outside the genesis entry a hard MUST-reject
 * ("Setting portable: true is permitted only in the first entry"), so any
 * later entry unconditionally carrying `portable: true` forward (biset's
 * default) was a real interop break — found via a resolve_file() check
 * against didwebvh-rs, the DIF reference implementation (2026-07-28), which
 * rejects exactly that. `resolved` is resolveParameters(previous, current);
 * passing it in rather than recomputing keeps this a pure diff. */
