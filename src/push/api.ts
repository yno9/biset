...
// `path` is overridable for the DIDComm mediator, which serves the SAME key at
// /didcomm/push/vapid-public-key (anchor/mediator/server.ts). A relay-less
// identity (DID⊥relay) has no relay to ask, and it is the one that needs push
// most — everything it receives arrives through the mediator.
export async function fetchVapidPublicKey(serverUrl: string, path = '/jmap/push/vapid-public-key'): Promise<string> {
  try {
