...
- [~] **0.8 テスト** (agent: /root, 開始: 2026-09-01): `test/mimi/store.test.ts`, `test/mimi/http.test.ts`, `test/mimi/wire.test.ts`。biset内で3台のクライアントがroom作成→member追加→KeyPackage交換→**0.5.1経由でcommitを受信する**ところまで一通り行えることをE2Eで確認する。
