...
import { pickupDeliver, acknowledgeMessages } from '../src/did/didcomm/pickup.ts'
...
console.log('\n=== ack 前は再取得で残る（Pickup 3.0 非破壊）===')
const beforeAckA = await pickupDeliver(info, deviceA, resolveSenderKeyForBob)
ok('ack前は端末Aで再取得できる', beforeAckA.length === 1)

console.log('\n=== messages-received でack後は両方とも空（各kid独立に削除）===')
await acknowledgeMessages(info, deviceA, deliveredToA.map(d => d.ackId))
await acknowledgeMessages(info, deviceB, deliveredToB.map(d => d.ackId))
...
ok('端末Aはack後は空', secondA.length === 0)
ok('端末Bもack後は空', secondB.length === 0)
