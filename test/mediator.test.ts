...
import { identityFromKeys, generatePeerIdentity } from '../src/did/peer/peer.ts'
...
import { decodePeerDid2 } from '../src/did/peer/peer.ts'
...
// AFTER the keylist-update, not before: a pickup for a kid this connection
// hasn't registered is itself refused now (e.p.req.not_enroll — see
// mediator-queue-ownership.test.ts for why that gate exists), so "was the
// refused forward queued?" can only be asked by someone entitled to the answer.
// The property under test is unchanged: nothing was queued by the refusal.
...
// Pickup 3.0: delivery is NON-destructive — the message stays queued until the
// recipient acks it with messages-received.
ok('delivery 後もキューに残る（非破壊）', (await pickupStatus(info, aliceSender)) === 1)
const remaining = await acknowledgeMessages(info, aliceSender, got.map(g => g.ackId))
ok('messages-received 後にキューが空', remaining === 0 && (await pickupStatus(info, aliceSender)) === 0)
