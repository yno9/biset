...
// conversation-group-ingress.ts uses) and that its content-addressed
// messageId matches what the OWN-copy projection
// (mimi-content-projector.ts) would file it under, so a sender's own Vault
// write and a recipient's ingested copy agree on the same id.
...
import { addMembersToConversationGroup, createConversationGroup, receiveConversationEntry } from '../../src/mls/conversation-group.ts'
...
describe('sendConversationTextMessage', () => {
  test('composes a MimiContent, submits it, and bob decrypts the identical content -- both sides agree on messageId', async () => {
    ...
    const welcomeEntry = ds.deliveriesSince(groupId, bob.kid, 0)!.find(e => e.kind === 'welcome')!
    const bobState = await joinMlsGroup(welcomeEntry.payload, bob.own, undefined)
    const beforeSend = ds.deliveriesSince(groupId, bob.kid, 0)!.length

    const sent = await sendConversationTextMessage({ state: aliceState, transport, groupId, deviceKid: alice.kid, text: 'hello from egress', sign: aliceSign })
    ...
    const entries = ds.deliveriesSince(groupId, bob.kid, beforeSend)!
