...
import { generatePeerIdentity } from '../src/did/peer/peer.ts'
...
import type { PeerDidDoc } from '../src/did/peer/peer.ts'
import { b64url } from '../src/did/peer/peer.ts'
import { didCommToEmail } from '../src/did/didcomm/channel.ts'
import { addSession, accountKey, DIDCOMM_SERVER_URL, sessions, sessionForRelay, identityKey, identityKeyForEmail } from '../src/context.ts'
import * as messages from '../src/store/messages.ts'
import { loadInboxSummaries, getInboxEmails } from '../src/app.ts'
import * as contactsStore from '../src/store/contacts.ts'
import { buildCardForDid, allKnownAddressesFor, contactIdentityKey } from '../src/did/contacts.ts'
...
console.log('\n=== #new コンポーザの送信後処理 — DID宛送信でクラッシュせず、作った会話を引き当てる ===')
// The composer sends to a DID with `relayUrl = undefined` (a DID recipient has
// no relay to route through), then immediately looks the new conversation up.
// Both halves used to break for exactly the shape Carol has (relay-backed
// identity + DIDComm):
//   1. sessionForRelay(from, undefined) dereferenced the missing serverUrl and
//      THREW — inside the async send handler, after the message had already
//      gone out, so hideCmdPalette/loadLeftInboxes/switchInbox never ran. The
//      recipient got the message; the sender's UI never gained the row at all.
//   2. the lookup compared the summary's `user` to the sending session's own
//      address. loadInboxSummaries sets `user` from the identity's FIRST
//      endpoint (Carol's relay address), while the DIDComm send resolves to the
//      synthetic session whose address is the DID — never equal, so even
//      without the throw the row was missed and a hand-built summary with the
//      wrong mailbox opened an empty thread.
// (2026-07-28, user-reported: "新規didメッセージをおくっても新規インボックスが
// 作成されない（相手方には作成される）".)
let threw = false
let relaylessSess: unknown
try { relaylessSess = sessionForRelay(CAROL_RELAY_EMAIL, undefined) } catch { threw = true }
ok('relay無し(DID宛)の sessionForRelay が throw しない', !threw)
ok('relay無しでは undefined を返す（呼び出し側の ?? sessionFor へ落ちる）', relaylessSess === undefined)

const carolDidCommSess = sessions.find(s => s.account.did === CAROL_DID && s.account.serverUrl === DIDCOMM_SERVER_URL)!
const composerIdKey = identityKey(carolDidCommSess)
const composerMatch = carolInboxes.find(i => identityKeyForEmail(i.user) === composerIdKey && contactIdentityKey(i.contact) === contactIdentityKey(bob.did))
ok('送信直後の行引き当てが identity 基準で当たる', composerMatch?.user === CAROL_RELAY_EMAIL && composerMatch?.contact === bob.did, JSON.stringify(composerMatch))
ok('旧実装の literal 比較では当たらなかったこと自体も固定', !carolInboxes.some(i => i.user === carolDidCommSess.account.email && i.contact === bob.did))
if (composerMatch) {
  // The whole point of finding the real row: its mailbox is the one
  // getInboxEmails needs. The hand-built fallback used currentInbox's mailbox.
  ok('引き当てた行でスレッドが開ける（mailbox が正しい）',
    getInboxEmails(composerMatch.mailbox, composerMatch.contact, CAROL_RELAY_EMAIL, CAROL_DID).length === 1)
}
...
console.log('\n=== 相手のDIDが Card として解決済みでもスレッドが空にならない（did/contacts.ts 回帰）===')
// Every getInboxEmails() check above ran with an EMPTY contactsStore, which is
// the one state that can't expose this bug: allKnownAddressesFor() fell
// through to `[key]` and the raw DID matched. Once discovery resolves the
// peer's DID document into a Card (the normal steady state — a Card keeps the
// DID in cryptoKeys and real addresses in emails), addressesForContactKey used
// to return ONLY the Card's addresses, dropping the DID a DIDComm message is
// actually addressed to — so getInboxEmails filtered out the entire
// conversation while loadInboxSummaries (which never goes through that helper)
// still listed the row. Symptom in production: the inbox row shows its preview
// but opens to an empty thread (2026-07-28).
contactsStore.put(buildCardForDid(bob.did, [
  { did: bob.did, address: 'bob@relay.example', relays: [], name: 'Bob' },
]))
ok('Card解決後も DID 自身がアドレス集合に残る', allKnownAddressesFor(bob.did).includes(bob.did), JSON.stringify(allKnownAddressesFor(bob.did)))
const cardInboxes = await loadInboxSummaries()
const cardRow = cardInboxes.find(i => i.user === ALICE_DID && i.contact === bob.did)
ok('Card解決後も Bob との会話行は出る', !!cardRow, JSON.stringify(cardInboxes.map(i => [i.user, i.contact])))
if (cardRow) {
  const cardEmails = getInboxEmails(cardRow.mailbox, cardRow.contact, ALICE_DID, ALICE_DID)
  ok('Card解決後もスレッドが空にならない', cardEmails.length > 0, `got ${cardEmails.length}`)
}
