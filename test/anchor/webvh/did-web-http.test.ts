...
import { createDidWebHttpHandler } from '../../../src/anchor/webvh/did-web-http.ts'
import { DidWebStore } from '../../../src/anchor/webvh/did-web-store.ts'
import { encodeMultikey } from '../../../src/identity/webvh/multikey.ts'
import { multikeyHashBase58 } from '../../../src/identity/webvh/hash.ts'

function spareHash(): string {
  return multikeyHashBase58(encodeMultikey(ed25519.getPublicKey(ed25519.utils.randomSecretKey())))
}
...
describe('createDidWebHttpHandler', () => {
  ...
  test('syncDidWebMirror publishes what buildMinimalWebvhState + resolveEntries reconstruct, GET returns it', () => withStores(async (webvh, didWeb) => {
    ...
    const { did } = await createGenesis({ domain: 'alice.test.example', rootPrivateKey, rootPublicKey, nextKeyHash: spareHash(), fetch: fetchImpl })
    const state = buildMinimalWebvhState(did, rootPublicKey)
    state.service = [{ id: `${did}#routing`, type: 'BisetRoutingDocument', serviceEndpoint: 'https://alice.test.example/.well-known/routing.json' }]
  ...
  test('a mirror write not matching the current did:webvh state is rejected', () => withStores(async (webvh, didWeb) => {
    ...
    const rootPrivateKey = ed25519.utils.randomSecretKey()
    const rootPublicKey = ed25519.getPublicKey(rootPrivateKey)
    await createGenesis({ domain: 'alice.test.example', rootPrivateKey, rootPublicKey, nextKeyHash: spareHash(), fetch: fetchImpl })
