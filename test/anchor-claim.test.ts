...
ok('by-did が victim を指す', (await byDid(victim.did).then(r => r.json()))?.localpart === 'victim')
...
ok('by-did は victim を指したまま', (await byDid(victim.did).then(r => r.json()))?.localpart === 'victim',
  '乗っ取りが通ると、ここが attacker に書き換わる')
