...
import type { IngressAckAuthorizer } from '../../src/core/mediation/ingress-store.ts'
...
const authorizer: IngressAckAuthorizer = { async isTrustedDevice(_identity, deviceId) { return ['device-a', 'device-b'].includes(deviceId) }, async verify() { return true } }
...
describe('SQLite ingress store', () => {
  test('survives restart only while the external body remains pending', async () => {
    ...
    expect(await restarted.pull(identityId, 'device-b', new Date('2026-08-21T01:00:00.000Z'))).toEqual([envelope])
  ...
  test('does not resurrect envelope metadata or body after ACK across a restart', async () => {
    ...
    first.close()
    const restarted = SqliteIngressStore.open(path, authorizer)
    expect(await restarted.pull(identityId, 'device-b', new Date('2026-08-21T02:00:00.000Z'))).toEqual([])
  ...
  test('expires a body into a tombstone under the same bounded policy', async () => {
    ...
    expect(await store.pull(identityId, 'device-a', new Date('2026-08-21T00:02:00.000Z'))).toEqual([])
    store.close()
  })
})
