...
} from '../../src/mail-mediator/smtp-protocol.ts'
import type { AcceptIngressInput, SmtpEffect, SmtpSessionDeps } from '../../src/mail-mediator/smtp-protocol.ts'
...
interface Fixture {
  ...
  resolveResult: Map<string, boolean | 'throw'>
...
function makeSession(overrides: Partial<SmtpSessionDeps> = {}): Fixture {
  ...
  const resolveResult = new Map<string, boolean | 'throw'>()
...
describe('SmtpSession: RCPT resolution', () => {
  test('a resolvable recipient is accepted 250', async () => {
    const fx = makeSession()
    fx.resolveResult.set('alice@mail.test.example', true)
  ...
  test('non-ASCII recipient address is rejected without calling resolveRecipient', async () => {
    let resolveCalls = 0
    const fx = makeSession({ async resolveRecipient() { resolveCalls += 1; return true } })
...
describe('SmtpSession: DATA', () => {
  ...
  test('calls acceptIngress exactly once per surviving recipient with the same body', async () => {
    const fx = makeSession()
    fx.resolveResult.set('alice@mail.test.example', true)
    fx.resolveResult.set('bob@mail.test.example', true)
