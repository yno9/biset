...
`src/vault/recovery-archive.ts` はこの archive の endpoint-only AES-GCM envelope と canonical file representation を定義する。file は outer ciphertext/envelope だけを持ち、recovery key、plaintext vault、MLS secret、端末 signing key を含まない。encrypted body は manifest、元の event/object ciphertext、各 object を開く SegmentKey を含む。`recovery-archive-export.ts` は IndexedDB の local record reader からその完全 snapshot を作る。`recovery-archive-rewrap.ts` は復元後の current self group / current epoch からだけ新しい `SegmentKeyWrapV1` を発行する。`recovery-archive-import.ts` は raw event/object と fresh wrap を一つの local transaction へ渡し、復元後の JMAP projection rebuild を別の必須段階に残す。VEK buffer は re-wrap 後に消去する。browser file picker/download UI と projection rebuild は後続工程である。
