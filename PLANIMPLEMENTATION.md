...
`src/vault/recovery-archive.ts` はこの archive の endpoint-only AES-GCM envelope と canonical file representation を定義する。`recovery-archive-file.ts` はそれを browser Blob/File boundary にし、identity を filename に入れない。file は outer ciphertext/envelope だけを持ち、recovery key、plaintext vault、MLS secret、端末 signing key を含まない。encrypted body は manifest、元の event/object ciphertext、各 object を開く SegmentKey を含む。`recovery-archive-export.ts` は IndexedDB の local record reader からその完全 snapshot を作る。`recovery-archive-rewrap.ts` は復元後の current self group / current epoch からだけ新しい `SegmentKeyWrapV1` を発行する。`recovery-archive-import.ts` は raw event/object と fresh wrap を一つの local transaction へ渡し、復元後の JMAP projection rebuild を別の必須段階に残す。VEK buffer は re-wrap 後に消去する。browser file picker/download UI と projection rebuild は後続工程である。
...
現在 `src/local-jmap/mutations.ts` は `Email/set` の `mailboxIds`、`keywords`、`destroy` を `mailbox.set`、`keyword.set`、`message.tombstone` の `VaultMutationIntent` に正規化する。`src/vault/mutations.ts` は通常 intent payload を encrypted `VaultObjectV1` とし、その object ID を参照する signed event を組み立てる。object AAD は identity / segment / event kind / target に束縛する。次の vault transaction がこの二つと projection/checkpoint/outbox を一原子的 commit にする。

mail の `message.add` はこの一 object 規則の意図的な例外である。`src/vault/mail-message.ts` は同じ署名 event の `objectRefs` を順序付きで `[metadataObjectId, rawRfc5322ObjectId]` とする。第1 object は canonical JMAP metadata（`email.id`、mailbox、subject 等）を持つ mutation object、第2 object は一切書き換えない raw RFC 5322 bytes であり、email の `blobId` は必ず第2 reference と等しい。raw object は別 AAD domain で暗号化するため mutation object と取り違えない。core は両 object を opaque ciphertext として短期配送するだけで、endpoint は event signature・各 object の content integrity・metadata/blob binding を検証する。これは JMAP projection の再構築と `Email/get` の blob download を可能にしながら、mail 原本を端末 vault の正本として残す。attachment / decrypted MIME projector / `Email/import` UI transaction は後続工程である。

`src/local-jmap/reducer.ts` は復号・event signature 検証済みの mutation object を event metadata と照合してから projection に適用する。競合する state event は `(createdAt, actorDeviceId, actorSeq, eventId)` の昇順で処理するため全端末で決定的に収束する。`message.tombstone` は tombstone set に残り、並行する mailbox/keyword 更新で message を再出現させない。`message.add` は上記の二 object binding を満たすときだけ email を追加し、mailbox total/unread counts を現在の email 集合から再計算する。attachment / reply / thread reducer は後続工程である。
...
5. **vault への確定**: A は raw `.eml` object、復号後の message / attachment object、検証結果、contact-key / group / reaction event、Local JMAP projection を durable に保存する。raw mail は export、再解析、再検証、相互運用のために端末 vault に残す。現在の `MailIngressProjector` の第一段階は、core から受け取った opaque raw body をそのまま raw object とし、最小の Inbox JMAP metadata を持つ `message.add` と一つの署名 event に束縛する。MIME / OpenPGP の解釈結果・attachment は後続 projector が追加する。
6. **ACK と sibling 同期**: A は `IngressAckV1` を送る。mediator は有効性を確認後に raw mail の短期 body を削除する。B/C へは SMTP で再配送せず、vault delivery と MLS による端末認可で同期する。`IngressVaultCommit` は message records と projection、ingress ACK outbox に加え、同じ event を entry ID とする shared vault-delivery outbox も同一 IndexedDB transaction に書く。したがって ACK 後に sibling 同期だけが失われる状態を作らない。
