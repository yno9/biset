...
`src/vault/recovery-archive.ts` はこの archive の endpoint-only AES-GCM envelope を定義する。encrypted body は manifest、元の event/object ciphertext、各 object を開く SegmentKey を含む。MLS exporter secret・VEK・端末 signing key は入れない。復元後は archive 内の SegmentKey を使って既存 object を検証・読出しでき、新しい self group / current epoch が確立した時点で新しい `SegmentKeyWrapV1` を作る。現段階は cryptographic archive primitive までであり、IndexedDB からの完全 export/import とこの post-import rewrap は後続工程である。
