...
- **Bottom safe-area (home-indicator) black band — the mirror of the top notch, and iOS lies about the viewport height.** Symptom: in the standalone PWA a ~34px strip below the reply-dock (and below the left pane) paints as bare **webview black** — not even `var(--bg)` (`#0d0d0d`) — from first paint, at all scroll positions. Measured on device: `window.innerHeight === document.documentElement.clientHeight === 100dvh === 793`, yet `env(safe-area-inset-bottom) === 34` and the physical screen is 827. **iOS reports the *safe* viewport (793) as the full viewport**, so anything sized to `100dvh` (`#app`) or `inset:0` inside it (`#left-pane`, `#right-col`, both `position:absolute` on mobile) stops at 793; the 34px below is outside the document → OS black. Fix (scoped `@media (max-width:520px) html.pwa-standalone`): grow `#app` to `calc(100dvh + env(safe-area-inset-bottom))` so both panes reach the true bottom and cover the band, then add `env(safe-area-inset-bottom)` to the bottom padding of `.reply-box` and `#left-pane` so controls/list-tail stay above the indicator while the frosted backgrounds fill behind it. **Red herring to not chase again:** `#reply-dock` is `position:fixed; bottom:0` but is **transform-trapped** by `#right-col`'s `transform: translateX(...)` (the mobile slide), so it anchors to that box (793), not the viewport — toggling the dock between `fixed` and `absolute` changes *nothing* here (both resolve to 793). The bug is `#app`'s height, not the dock's positioning.
