...
- **Portable identity (DID)** — a `did:webvh` root, generated client-side, backed up as a 24-word recovery phrase. Move relays, add a relay run by a different operator, or bring your own domain, and your identity — keys, contacts, discoverability — comes with you. DID is layered on top of, not instead of, real protocols: biset still speaks plain SMTP/JMAP/ActivityPub, so nothing about interop is sacrificed for portability. A mail account can still run with no DID at all ("anchorless"); a `did:webvh` identity cannot, since its genesis is published to the anchor. See [`ARC.md`](ARC.md#identity-layer-did).
