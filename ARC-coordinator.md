...
2026-08-30に個別device revokeをSign Key rotationベースの一括世代交代へ置き換えた。Anchorは現行Sign keyで証明されたWebVH `versionId`をlogin credentialとOAuth access/refresh tokenへ継承する。Coordinator v2はDID/SCIDを受け取らず、pairwise subjectごとにこのopaque generationを単調増加させる。新世代を一度受理した後は、旧access tokenを`401 invalid_token`で拒否する。rotation実行Clientは旧refresh sessionを破棄し、Vault cardを`Reconnect required`にするため、一度の標準OIDC再接続でserver-side世代交代が確定する。
