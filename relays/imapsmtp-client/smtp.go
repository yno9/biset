...
func smtpDeliver(cfg SMTPConfig, from string, to []string, raw []byte) error {
...
func smtpData(c *smtp.Client, from string, to []string, raw []byte) error {
...
func buildRFC5322(from string, to, cc []string, subject, body, msgID, inReplyTo string) []byte {
...
func newMsgID(addr string) string {
	b := make([]byte, 8)
