...
func imapFetch(cfg IMAPConfig, inboxKey string, since AccountState) ([]vault.Message, AccountState, error) {
	c, err := imapDial(cfg)
	if err != nil {
	...
	msgUIDs := since.EmailUIDs
	if msgUIDs == nil {
		msgUIDs = map[string]uint32{}
	}
	if len(newUIDs) > 0 {
		messages, maxUID = fetchUIDs(c, newUIDs, cfg.Username, inboxKey, msgUIDs)
	}
	if maxUID == 0 {
	...
	sentMsgs, sentState := fetchSent(cfg, inboxKey, since, msgUIDs)
...
func fetchSent(cfg IMAPConfig, inboxKey string, since AccountState, msgUIDs map[string]uint32) ([]vault.Message, AccountState) {
	c, err := imapDial(cfg)
	if err != nil {
		return nil, since
	}
...
func imapHandle(cfg IMAPConfig, state AccountState, msgID, action string) error {
	...
	case "seen":
		...
		return fmt.Errorf("cannot determine message ID for %q", msgID)
...
// parseRawMessage converts a raw MIME email into a vault Message (JMAP Email).
func parseRawMessage(raw []byte, selfAddr, inboxKey string, seen bool) (*vault.Message, error) {
	...
	go processAutocrypt(os.Getenv("BISET_VAULT"), fromAddr, raw)
