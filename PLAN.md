...
**次に着手する工程:** §4.1 は DID resolver・MLS 暗号処理系の移植・roster install 認可・roster producer・`MlsSelfGroupProvider`・AS・self-group DS 通信（12 操作、core + client transport）・self-group bootstrap（`ensureSelfGroup`）・roster 取得 API と roster install の接続（`ensureSelfGroupWithRosterInstall`/`installCurrentRosterProjection`）・既存メンバーによる新 epoch 検知/反映（`reflectPendingSelfGroupCommits`）・did:web mirror（`src/identity/web/`）・key-package pool（`keypackage-store.ts`/`key-package-pool.ts`）・新規 identity 作成の end-to-end bootstrap（`identity/bootstrap.ts` の `createNewIdentity`）と最小限の new-user UI（`src/ui/`、`src/main.ts` からの結線）まで実装した。残るのは (1) ログイン/mnemonic からの復元（既存 identity での再参加）、(2) `ensureSelfGroup`/`reflectPendingSelfGroupCommits`/`ensureKeyPackagePool` の定期実行タイミング、(3) `src.bak/ui/` の残り（left-pane・account 管理・relay 接続等、9000 行超）の本格移植 — これらと `submitApplication` を移植するかどうかの判断は、Vault Core 全体のブートストラップに関わる別の大きな作業として後続に残す。§3.3 の ingress/vault 接続とは独立して進められる。ingress は generic public HTTP API にせず、first-party adapter の内部 boundary に限定する方針は維持する。
...
- [x] identity bootstrap（新規 identity 作成のみ、`src/identity/bootstrap.ts` の `createNewIdentity`）を実装した。root key 導出（`identity/keys.ts`/`identity/seed.ts`、`src.bak/did/keys.ts`/`seed.ts` の SLIP-10/BIP39 をそのまま移植）→ did:webvh genesis（`createGenesis`、`didWebMirror: true`）→この device の MLS leaf key を verificationMethod として登録（`addDeviceVerificationMethod`）→ `ensureSelfGroupWithRosterInstall` で self-group 参加 + roster 反映 → `ensureKeyPackagePool` で補充、まで一気通貫。identity のローカル永続化は `src/identity/record-store.ts`（`IndexedDbIdentityRecordStore`、旧 `DidRecord` から mail/AP/pre-rotation/JMAP デバイス鍵を落とした最小形、**平文保存** — passkey 封印は未移植、後で `sealed` フィールドに差し替えられる形だけ用意）。`setMlsAuthService` はこの呼び出し内で一度だけ配線する（グローバル状態、group.ts 自身の注記通り）。`selfGroupStore`/`keyStore` は呼び出し元が注入するため、実 DID anchor + 実 core（`createBisetCoreFetchHandler`）+ 実 DID 解決（`WebvhSigningKeyResolver`）を通した end-to-end test（`test/protocol/identity-bootstrap.test.ts`）がブラウザ外でも走る。ログイン/mnemonic からの復元（`src.bak/did/restore.ts`）は未移植 — 新規作成のみ。
