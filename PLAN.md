...
- [-] outbound SMTP client（`src/core/adapters/mail-smtp-client.ts`、MX解決・client STARTTLS・RFC 5321 dialogue）を実装し、実装済みの inbound listener への実配送で実証した。**意図的にここまで**——outbound intent の認証付き narrow-API（device→core、まだ無い）・device 側 transport・vault 側 outbox 管理（新しい event kind は不要と判明：`message.add` を "outbox" mailbox へ＋既存 reserved 済み未使用の `transport.result` kind で足りる）・`EmailSubmission/set` JMAP 配線・client-side PGP/MIME（相手の PGP 公開鍵取得手段が新設計に無く、DIDComm の wire format と同種の別途設計が必要と判明、ユーザーと協議のうえ後回しに確定）は全て未着手のまま次の作業として残す。
