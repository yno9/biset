...
**→ 不要と判明、クローズ(2026-07-16)。** この課題は「DIDCommは常に
did:peer経由」と考えていた時期の残骸だった。実際には:

- **biset↔biset間ではdid:peerそのものが不要**——did:dht直通が動いている
  (didmediatorがdid:dhtを解決できるようになった)。
- **そして交換すべきものが何も無い**: bisetは既に
  **アドレス→(DNSアンカー `_did.<localpart>.<domain>`)→did:dht→
  (DHT解決)→`_k1`+DIDCommMessagingサービス**という経路を持っている
  (`discovery.ts`の`addressToDid`と`didcomm/resolve.ts`が既に繋がっている)。
  **mail/APで接触した時点で、追加の交換なしにDIDCommできる。**
- **biset↔第三者実装(did:peerが要る唯一のケース)ではmail/APが使えない**
  ——相手はmailユーザーではないので、そもそもこの経路に乗らない。

**さらに、DIDをmail/APヘッダで交換する(Autocrypt方式)のは積極的に
避けるべき**と整理できた。`verifyBinding`のコメントが理由を明示している:
DID文書は自己署名なので他人のアドレスでも主張でき、**主張が信頼できるのは
そのアドレスが逆方向に同じDIDを指し返した時だけ**。DNSアンカーが効くのは
**ドメインという第三者が証言している**からで、これをヘッダに置き換えると
両方向とも送信者自身の主張になり、**双方向検証が循環して無意味化する**
(Malloryが「私はy@biset.md」と名乗り、自分のDID文書に
`aka=mailto:y@biset.md`と書けば両方向"一致"してしまう)。ヘッダは
DNS往復を省くヒントにはできるが、アンカーの代替にはならない。
...
## BEP44の1000バイト上限を撤廃: 継続レコード連鎖(実装完了・実DHTで検証済み、2026-07-16)

**問題**: BEP44の`v`は1000バイト上限。`_k1`+DIDCommサービスが載ると
relayは**4本**しか入らず、超えた瞬間に署名処理がthrow →`publishOne`が
握り潰す →**警告ゼロのまま再公開が止まり、約2時間でDHTから消えて
解決不能**になる。「relayを20個設定したユーザーはどうなる」という問いから
出発し、根本解決を実装した。

**採用: 継続DID連鎖。** 溢れたserviceを別のdid:dhtレコードへ追い出し、
root文書の`ext=`フィールドでそれを名指しする。`resolve()`が透過的に辿って
マージするので、**呼び出し側は1つの論理文書しか見ない**(`src/did/chain.ts`
が新規、`resolver.ts`の`resolve`/`publishDocument`が内部で使う)。

**なぜ他の案ではないか(検討順):**
- **BEP44 salt** — BEP44自身が「単一の鍵でいくつでもアイテムを公開できる」
  用途として明記しており、最初はこれが答えだと考えた。しかし**Pkarrの
  relayプロトコルにsaltフィールドが無い**(`3:seqi<seq>e1:v<len>:<bytes>`と
  empty-salt正規形をハードコード)。ブラウザはUDPを話せずrelay経由必須
  なので使えない。採用するならdid:dht仕様とPkarr relay仕様の**両方**を
  独自拡張する必要があり、しかも拡張レコードはbiset自身のゲートウェイ
  からしか読めなくなる。
- **外部ストレージサーバー** — DHTという**コモンズ**(誰も資金を出さず、
  誰にも止められない。ARC.mdの"joint production"の節)を、**誰かが
  維持し続けなければ消えるサービス**と引き換えにすることになる。
- **継続DID連鎖(採用)** — **どちらの拡張も不要**。各レコードはごく普通の
  empty-saltのdid:dhtレコードで、**公開Pkarrゲートウェイでそのまま引ける**。

**信頼の鎖**: root文書(root鍵で署名)が継続DIDを名指しし、継続の中身は
その継続自身の鍵で署名される——鍵はそのDID自身が名乗っているので、
**end-to-endで自己証明的**。ゲートウェイはリンクを握り潰せても偽造は
できない(既存のresolverと同じ性質)。

**互換性**: 1レコードに収まる文書は**今までとバイト単位で同一**に公開
される(`ext=`なし、継続なし)。溢れた時だけ連鎖する。`ext=`を知らない
汎用did:dht resolverも、**有効な文書 + 先頭N個のservice**を得る
(graceful degradation——`proto=`/`addr=`/`ac=`/`rk=`と同じ姿勢)。

**継続鍵の導出**: masterSeedではなく**root秘密鍵**からHKDFで導出
(`keys.ts`の`deriveContinuationKey`)。`publishDocument`はrootPrivateKeyしか
持たない(store.tsの「seedは永続化しない」原則)ので、seedをこの層まで
引き回さずに済む。root鍵自体がseed由来なので、**24語での完全復元性は不変**。
SLIP-0010のpathにしなかったのは、ここにseedが無いから。

**実測(実物の公開DHT/Pkarrゲートウェイ、`scratch-chain-live-check.mjs`)**:
relay 20本 → 3レコード、root は2/2ゲートウェイが受理(公開71.6秒)、
**解決2.6秒で20本すべてを順序通り復元、全署名検証OK**。オフラインでは
100本→15レコードも検証済み(`scratch-chain-check.mjs`)。

### 副産物: `packet.ts`のサイズ上限チェックがずっと間違っていた(実測で発見・修正)

連鎖の検証中、994Bは通るのに1071Bのペイロードが**500**で弾かれた。
BEP44原文を読み直すと**「`v`の*bencodeされた形*が1000バイトを超えるput
を拒否してよい」**——bencodeは`<長さ>:<バイト列>`なので、999バイトの`v`は
**1003バイト**になり上限超過。`pkarr.pubky.org`へ実際にputして境界を
bisectした結果、**理論と1バイトも違わず一致**:

```
v=996B  bencoded=1000B → 204 OK
v=998B  bencoded=1002B → 500 REJECTED
v=999B  bencoded=1003B → 500 REJECTED
```

エラー文言が決定的だった: **`Publish query timed out with no responses
neither success or failure`**——DHTノードはエラーを返さず**黙って無視する**。
つまり従来の`if (dnsPacket.length >= 1000) throw`は**997〜999Bの文書を
通してしまい**、それはDHTに黙殺され、`publishOne`がさらに握り潰す——
今回と同じ沈黙のバグを既に埋め込んでいた。**真の上限は生の`v`で996B**。
`packet.ts`をbencode長基準の判定に修正、`maxPacketBytes()`をexport
(実測996と一致)。`chain.ts`は`buildSignedPayload`でサイズ判定するので
自動的に正しいパッキングになる。

### 沈黙そのものも修正(2026-07-16、上記の続き)

連鎖で容量超過という*原因*は消えたが、`publishOne`の`catch { return 0 }`は
**あらゆる失敗を「ゲートウェイ不達」と区別なく飲み込む**という防御の穴を
残していた。ここも塞いだ。

**`0`の意味を、無害な2ケースだけに限定**: (a)publishするものが無い
(DIDレコードなし/接続relayなし)、(b)正しい記録を全ゲートウェイが拒否した
(日常的な一過性の失敗)。**それ以外——文書が構築・署名できない、連鎖の
リンクを置けない——は`throw`する**。これらは「何かを直すまで永久に公開
できない」という質的に違う状態で、`0`と混ぜた結果が今回の
「無言でDHTから消える」だった。

**呼び出し側3箇所すべてを、その区別に合わせて修正:**
- `publishOwnDids`(起動時、UIなし) — identityごとに`console.error`。1つの
  失敗が他を止めないことは維持しつつ、痕跡は必ず残す。
- `/account`の「Republish to DHT」(`left-pane.ts`) — **失敗を全て
  "No gateway reachable"と表示していた**。容量超過でも署名エラーでも
  「ネットワークの問題」と嘘をつき、ユーザーを無関係なデバッグへ誘導する。
  実際の理由を表示するよう修正。
- 独自ドメイン追加後の公開(`custom-domain.ts`) — **"will retry
  automatically"と表示していた**が、構造的失敗なら自動再試行も同じく
  永久に失敗するので、この文言こそ最も誤り。理由を表示するよう修正。
- 表示名変更後の再公開(`left-pane.ts`) — 名前の保存自体は成功している
  best-effortなので握り潰し自体は妥当。ただし`console.error`は残す。

**検証(`scratch-publish-errors-check.mjs`)**: (1)到達不能ゲートウェイ→`0`
(throwしない)、(2)単一serviceが巨大すぎる文書→throw、(3)連鎖の継続を
置けない→throw、の3ケースで区別が効くことを確認。

**このセッションの通底する教訓**: 同じ「握り潰し」が、今回だけで3つの
別々の実害を生んだ——(a)容量超過の無言死、(b)"No gateway reachable"という
嘘の診断、(c)"will retry automatically"という嘘の慰め。いずれも
**スクリプト検証では絶対に見つからない**(例外時・時間をまたぐ操作・
UI文言の問題)。実ユーザーの実データと実操作で初めて表面化した。
