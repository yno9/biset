...
- [-] PGP private-key credential は vault / peer restore の対象、全端末喪失への備えはユーザー管理の暗号化 recovery archive、core の恒久 key blob は置かない、と policy を決定した。暗号化 credential object/event schema、local atomic commit/outbox、shared delivery の検証・保存は実装済み。restore approval、rotation / revocation、archive export/import は未実装。
