...
- [ ] 上記「既存メンバーが新 epoch を検知して roster を反映する」フロー自体（deliveries pull による catch-up、いつ `installCurrentRosterProjection` を呼ぶか）はまだ実装していない。テストでは `pullDeliveries` + `processIncoming` を直接呼んで代用しており、self-group.ts のモジュールコメントが明示的に除外している「pendingRemovals 追従」などと同様、実際の呼び出しタイミングの設計が必要。
