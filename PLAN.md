...
**現在地（2026-08-24 時点）:** §2〜§4 の中核（protocol foundation、mediator、MLS self group、vault crypto、restore transfer/archive）は実装・実 MLS/実 core を通した end-to-end test でほぼ全て検証済み。**vault UI（§7）は read-only inbox first slice（受信箱一覧＋スレッド閲覧）まで実装・`main.ts` から接続済み**——`src.bak/ui/` 9000 行超のうち compose/search/command palette/account設定/prerotation/custom-domain は未移植のまま。**Mail adapter（§6.2）は SMTP listener（実 socket・STARTTLS・recipient resolution）まで実装・`core/index.ts` から接続済み**——outbound送信・DKIM/ARC実検証・OpenPGP/Autocrypt projectorは未着手のまま。残っている大きな未着手領域は次の 3 つ: (1) **DIDComm adapter**（§6.1、ゼロから未着手、wire format 決定も adapter boundary 移行が先）と Mail adapter の残り（outbound、DKIM/ARC検証）、(2) **vault UI の compose/account設定等の残り**（§7、上記の未移植部分）、(3) **旧 relay からの migration**（§8、import tool 含め全項目未着手）。ingress は generic public HTTP API にせず、first-party adapter の内部 boundary に限定する方針は維持する。
