...
- [-] MLS VEK 導出・MLS self group・SegmentKey の object encryption・VEK を入力に取る wrap primitive・Local JMAP Gateway の read/write path（実 MLS device 署名込み、`test/protocol/identity-end-to-end-mail-sync.test.ts` で create→write→deliver→restore→project を通しで確認済み）は実装済み（この行も同様に古いまま放置されていたので訂正した）。DIDComm adapter はゼロから未着手。Mail adapter は endpoint 側の projector に加え SMTP listener（実 socket、STARTTLS込み）と recipient resolution も実装済み（§6.2）。読み取り側は `main.ts` から `buildLocalJmapReadModel` 経由で vault UI（§7 read-only inbox first slice）に接続済み。`Email/changes`/`Mailbox/set`/`EmailSubmission/set` と write path（`VaultBackedLocalJmapMutationSink`）の vault UI 配線（compose 未実装のため）は未実装のまま。

**現在地（2026-08-24 時点）:** §2〜§4 の中核（protocol foundation、mediator、MLS self group、vault crypto、restore transfer/archive）は実装・実 MLS/実 core を通した end-to-end test でほぼ全て検証済み。**vault UI（§7）は read-only inbox first slice（受信箱一覧＋スレッド閲覧）まで実装・`main.ts` から接続済み**——`src.bak/ui/` 9000 行超のうち compose/search/command palette/account設定/prerotation/custom-domain は未移植のまま。**Mail adapter（§6.2）は inbound（実 socket・STARTTLS・recipient resolution）と outbound（SMTP client・認証付き narrow-API・vault outbox 配線・最小 EmailSubmission/set）ともに実装済み、実サーバ vault→core→SMTP→受信側 pullable ingress の通しで検証済み**——残るのは DKIM/ARC実検証・OpenPGP/Autocrypt projector・client-side PGP/MIME（相手鍵取得手段が別途設計待ち）のみ。残っている大きな未着手領域は次の 3 つ: (1) **DIDComm adapter**（§6.1、ゼロから未着手、wire format 決定も adapter boundary 移行が先）と Mail adapter の残り（PGP、DKIM/ARC検証）、(2) **vault UI の compose/account設定等の残り**（§7、上記の未移植部分）、(3) **旧 relay からの migration**（§8、import tool 含め全項目未着手）。ingress は generic public HTTP API にせず、first-party adapter の内部 boundary に限定する方針は維持する。
...
- [x] protocol version negotiation と未知 field / version の互換性 policy を決めた（2026-08-24、ユーザーと協議）。**恒久方針**: `validate.ts` の `exactKeys()` による未知 field の無条件拒否を維持する——client/core は常に同一 commit から作られる前提（単一リポジトリ、mixed-version 運用を想定しない）であり、厳格な方が互換性バグを早期に検出できる。将来 wire format を変える必要が生じたら、緩和ではなく `version` 番号を上げて対応する。version negotiation 自体（複数 version 同時サポート）は当面実装しない。
...
- [x] tombstone retention / dedup retention / quota eviction の数値を policy として決めた（2026-08-24、ユーザーと協議）。vault delivery の `deliveryTtlMs` を 24 時間 → **30 日間**に変更（`vault-delivery-store.ts`/`sqlite-vault-delivery-store.ts` 両方）——長期不在端末でも通常 delivery で追いつける猶予を確保しつつ、mediator 保持を無制限にはしない、というトレードオフ。他の数値（`maxPayloadBytes`=25MB、`maxIdentityPayloadBytes`=100MB、`maxIdentityPendingItems`=128、ingress の `claimLeaseMs`=60秒 等）は現状の default のままで妥当と判断し変更なし。**設定の置き場所も決定**: PLANIMPLEMENTATION.md が示唆する「定数に埋め込まず versioned configuration にする」という分離は、本番運用が始まってから実際に値を変えたくなったタイミングでやる方が無駄がないと判断し、今は現状通り各 Store のコンストラクタ引数で override 可能な code 内 default constant のままとする。default TTL に依存していた既存 test（`vault-delivery-store.test.ts` の TTL expiry test）は明示的な短い TTL を渡す形に修正した。
...
- [x] immutable `VaultEventV1` の ID、署名対象、actor sequence を実装した。signer interface と canonical event ID は実装済み。**MLS/device signer 接続**: `VaultEventSigner`（`{deviceId, sign, verify}`）と `SegmentKeyWrapSigner` は構造的に同一の形をしているため、`buildVaultCryptoBoundary` が返す実 MLS-backed `signer`（`MlsMembershipSegmentKeyWrapSigner`）は特別な接続コードを書かずにそのまま `VaultBackedLocalJmapMutationSink` の `signer` として使える——ただしこの組み合わせを実 MLS self group で検証した test が無かった（既存 test は全て SHA-256 fixture signer を使用）。`test/protocol/identity-local-jmap-write-path.test.ts` で、`Email/set` が実 MLS leaf 鍵で署名されること、その署名が実 MLS-connected verifier（`buildVaultDeliveryProjector`、別インスタンス）で受理・projection まで通ることを確認した。`VaultBackedLocalJmapMutationSink` 自体はまだ `main.ts` から呼ばれていない（vault UI 未実装のため）。
...
- [-] event signature、parent reference、duplicate event、replay の validation を実装する。署名検証は ingest 境界（`VaultDeliveryProjector`/`ingress-ingest.ts`、`decryptVaultMutationRecords` 経由）で行い、store 自体は既検証済みの記録を信頼する設計——store で再検証しないのは意図的な層分離。**duplicate event / replay**: `commitIngress`（既存）に加え `commitLocalMutation`（client 自身の mutation 書き込み経路）も `vault_events` の実 unique keyPath（`.add()`、`.put()` ではない）で dedup されることを実 IndexedDB で確認した（`test/protocol/vault-store-durability.test.ts`）——replay（既に適用済みの event が別経路で再度届く）も同じ機構で扱われるため追加のテストは行っていない。**parent reference は未実装のまま、gap として明記**: `VaultEventV1.parents` は署名対象に含まれ改変不可能だが、reducer の適用順序は `createdAt`→`actorDeviceId`→`actorSeq`→`id` の LWW sort のみで決まり（`local-jmap/reducer.ts`）、`parents` を実際の causal ordering や存在チェックに使うコードはどこにも無い（vestigial field）。意図的な設計判断か単なる未着手かは要判断——本格的な causal ordering を導入するかどうかは別途設計が必要。
- [-] 実装済みの kind（`message.add`/`message.tombstone`/`mailbox.set`/`keyword.set`）の競合規則を固定した: LWW（`createdAt`→`actorDeviceId`→`actorSeq`→`id` 順に sort してから適用）、tombstone は他の全 mutation に優先し不可逆、同一 `message.add` の重複は conflict として拒否（silent 上書きしない）。**この作業で発見した gap を修正**: reducer は `mailbox.set`/`keyword.set` 以外の未知 kind（`VaultEventKind` が予約している `message.edit`/`reaction.set`/`read.set`/`thread.set`/`settings.set` 等、まだどの書き込み経路も produce しない）を黙って無視していた——sync は成功したように見えるのに mutation だけ消える、検出不能なデータロス。「投影規則の無い kind は fail closed」に変更し、`test/protocol/local-jmap-reducer.test.ts` で確認した。edit/read/reaction/thread/settings 自体の kind ごとの規則（payload 形状・merge semantics）はまだ未設計・未実装のまま。
...
- [x] new core 用の最小 `MlsSelfGroupProvider` / fixed VEK exporter boundary を抽出した。`ClientState` から作る具象実装（`mls/store.ts` の `StoredMlsSelfGroupProvider`、`selfGroupStore.load` から `MlsEpochExporter`（`selfGroupId`/`epoch`/`exportSecret`）を作る）も実装済み——本チェックリストの記述が古いままだったのを訂正した。`identity/bootstrap.ts` の全 boundary 関数（`buildVaultCryptoBoundary`/`buildRestoreTransferSource`/`buildVaultDeliveryProjector`/`buildLocalJmapProjectionRebuild`/`buildVaultBlobReader`）が `new MlsVaultEpochKeyResolver(new StoredMlsSelfGroupProvider(selfGroupStore))` として使っており、`identity-*.test.ts` 群の実 MLS self group end-to-end test 全てで間接的に検証済み。
...
- [x] VEK を永続化しないことを code review / test で保証した。設計上そもそも VEK は `VaultEpochKeyResolver.deriveVaultEpochKey` の戻り値としてのみ存在し、`IndexedDbVaultStore` のどの書き込みメソッド（`writeSegmentKeyWrap`/`commitLocalMutation`/`commitDelivery` 等）も VEK 形の値を受け取る field を持たない——保存されるのは常に AEAD 済みの `SegmentKeyWrapV1`（wrapped SegmentKey）であり、VEK 自体ではない。read 側（`StoredSegmentKeyResolver.resolveSegmentKey`、`test/protocol/segment-key-resolver.test.ts` で既に検証済み）と write 側（`ActiveVaultSegmentManager.mintWrap`、新設 `test/protocol/active-segment.test.ts`）の双方で、導出した VEK を capture できる `VaultEpochKeyResolver` fake を使い、呼び出し完了後に VEK バッファが `fill(0)` 済み（全ゼロ）であることを確認した。残る呼び出し箇所（`identity/bootstrap.ts` の `selfGrantSegmentRewraps`/`buildRestoreTransferSource.readCurrentEpochWraps`、`vault/recovery-archive-rewrap.ts`）は grep で `finally { vek.fill(0) }` パターンの存在を code review 済み。
...
- [x] `src.bak/ui/account-create.ts`（署名フォーム）・`mnemonic.ts`（recovery phrase 表示）の TS ロジックを流用し、`src/ui/account-create.ts`/`src/ui/mnemonic.ts` として新設計向けに書き直した。DNS anchor lookup によるログイン分岐・mail/AP relay provisioning・DIDComm mediator 登録・TOS 同意・passkey enrollment はすべて未移植（対応する新実装がまだ無いか、スコープ外）。`src/main.ts` の `bootClient()` から、ローカルに identity が 0 件なら `setupNewUserPage()` を呼ぶ形で結線した（1 件以上ある場合は §7 の read-only inbox first slice に接続済み、下記参照）。
...
- [-] manifest first、chunk hash、resume cursor を使う peer transfer frame の作成・検証と、verified records/session cursor の IndexedDB atomic import を実装した。**projection rebuild は実は既に実装済みだった**——§5.2 で作った `rebuildLocalJmapProjection`（`vault/projection-rebuild.ts`）は identity の全 record を対象にする record-source-agnostic な設計で、restore-transfer が commit した record にもそのまま使える。実際にそれを実 MLS self group + 実 restore-transfer round trip（`createRestoreTransferChunk`→`verifyRestoreTransferChunk`→`receiveRestoreTransferChunk` で commit）で確認していなかったため、`test/protocol/identity-restore-transfer-projection-rebuild.test.ts` を追加し、restore-transfer が commit した raw record から `buildLocalJmapProjectionRebuild` が正しい projection（実際の `message.add` の email を含む）を再構築することを確認した。実際の direct/relayed channel と browser fault test は依然未実装。
...
- [-] user-owned archive の endpoint-only AES-GCM envelope と、IndexedDB record reader から全 event/object と必要な SegmentKey を集める export snapshot builder を実装した。復元後の current MLS epoch だけへ SegmentKey を再 wrap し、raw record/wrap を atomic import する endpoint workflow を実装した。encrypted-only canonical file / Blob boundary も実装した。**projection rebuild を確認した**: `commitRecoveryArchive`（`IndexedDbVaultStore`）は restore-transfer の commit と同じ `vault_events`/`vault_objects`/`vault_key_wraps` store に書くため `rebuildLocalJmapProjection`（record-source-agnostic）はそのまま archive import 後にも使えるはずと理屈では分かっていたが、実際には未確認だった。`test/protocol/identity-recovery-archive-projection-rebuild.test.ts` を追加し、実 MLS self group で archive を作成 → `importRecoveryArchive`（current epoch への re-wrap込み）→ `buildLocalJmapProjectionRebuild` の一連を通し、archive-imported record から正しい projection（実際の `message.add` の email を含む）が再構築されることを確認した。browser file picker/download UI は未実装。
...
- [-] SMTP input を raw RFC 5322 ingress に変換する実装を追加した（`src/core/adapters/mail-smtp-listener.ts`、`mail-smtp-protocol.ts`）。upstream JMAP input（他 JMAP サーバからの取り込み）は未着手のまま。
- [x] RCPT TO と address-to-identity resolution を実装した（`mail-recipient-resolver.ts`）。identity は subdomain-per-identity did:webvh 規約を前提に、`alice@mail.<apex>` → `resolveByDomain(alice.<apex>)` → `TrustedDeviceRoster.trustedDevices` のライブ解決で足り、別レジストリは不要だった。
- [-] raw mail、envelope を server-side decrypt せず渡す部分は実装済み（`rawRfc5322`/`smtpEnvelope` はそのまま `MailIngressAdapter.accept()` に渡る）。DKIM/ARC evidence は本物の検証が未実装のため、`sourceEvidence` は listener が実際に観測した接続由来情報（remote address/HELO domain/MAIL FROM/TLS state）の passthrough に留まる——DKIM/ARC 検証実装は別タスク。
- [x] endpoint の `MailIngressProjector` は opaque mail ingress を raw RFC 5322 object + encrypted JMAP metadata の `message.add` に変換し、ingress receipt / ACK outbox / sibling vault-delivery outbox を一 transaction に確定する。SMTP listener・address resolution は実装済み（上記）。RFC 5322/MIME metadata parser（Subject/Date/Message-ID/References の conservative 抽出のみ）・OpenPGP/Autocrypt projector は引き続き別項目のまま。
...
- [x] outbound 送信を実装した: outbound SMTP client（`mail-smtp-client.ts`、MX解決・client STARTTLS・RFC 5321 dialogue）、device→core 認証付き narrow-API（`protocol/mail-submission*.ts`・`core/adapters/mail-submission-adapter.ts`・`POST /v1/mail/submit`、既存の ingress/vault-delivery と同じ形）、device 側 transport（`vault/mail-submission-transport.ts`）、vault 側 outbox 管理（新しい event kind は不要、`message.add` を "outbox" mailbox へ＋既存 reserved 済み未使用の `transport.result` kind の reducer no-op 追加のみ）、`identity/bootstrap.ts` の `buildMailSubmitter` 境界関数、最小 `EmailSubmission/set` JMAP 配線（単一 create のみ、undoStatus polling 無し）。実サーバ vault → narrow-API → outbound SMTP → inbound listener → 受信側 pullable ingress を通しで実証（`test/core/adapters/mail-submission-e2e.test.ts`）。client-side PGP/MIME は引き続き未着手（相手の PGP 公開鍵取得手段が新設計に無く、DIDComm の wire format と同種の別途設計が必要、ユーザーと協議のうえ後回しに確定のまま）。
- [-] `250` 後の TTL expiry policy を決定した（2026-08-24、ユーザーと協議）。**短期受理型**（`offerIngress` 成功時点で `250` を返す）に確定——SMTP 側の即時完了を優先し、TTL 切れ時は Biset 側が DSN/permanent failure を発行して失敗を明示する責任を負う。理由: 送信側 MTA のリトライ挙動に依存する非受理型（4xx）より、受信側の状態がユーザーにも実装にも追いやすい。SMTP listener 側は実装済み（`mail-smtp-listener.ts`、TTL default 30日 = vault delivery と揃える、2026-08-24 ユーザーと協議）——DATA 完了で即 `accept()` を呼び `250` を返す。DSN 生成自体は引き続き未実装のまま：`IngressStore.expire()`（memory/SQLite 両方）は既に期限切れ record を `IngressStatusRecord[]` として返すため、`protocol: 'mail'` の expired record を拾って DSN を生成するフックポイントは既存 API で足りる、追加の store 側変更は不要と判断した。
...
- [x] Read-only inbox first slice を実装した（`src.bak/ui/` の完全移植ではなく、受信箱一覧＋スレッド閲覧のみに絞った意図的な部分移植）。`src/mail/message-view.ts`（`threading.ts` の union-find を無変更で移植、新規 `emailToMessageView` で `LocalJmapEmail` のメタデータ＋生 RFC5322 blob から表示用データを合成）、`src/mail/body-text.ts`（新規、text/plain・multipart/alternative の本文抽出）、`src/ui/thread.ts`/`left-pane.ts`/`shell.ts`/`format.ts`（`thread.ts` のレンダリングロジックを移植、compose/edit/delete/reactions/attachments/DID・relay context は全て対象外——`left-pane.ts` は旧 `InboxSummary`（per-contact, multi-relay）ではなく新設計にある `ThreadGroup` を単位に作り直した、旧コードのデータモデル自体がこのrewriteに存在しないため）、`src/identity/bootstrap.ts` の `buildLocalJmapReadModel`（新規、既存の `buildVaultBlobReader`+`IndexedDbLocalJmapReadModel` を結線するだけの boundary 関数）、`src/main.ts` の `bootClient()` から `showApp`/`refreshInbox` へ結線。`src/index.html`/`style.css` は `src.bak` の該当 DOM/CSS 断片を、既存 `#app`（new-user-page 用 grid layout）と衝突しないよう新規 `#mail-app` として追加。`bun run build` + `dist/index.html` を file:// で開き、mock `LocalJmapReadModel`（実データ層は既存の実 MLS/実 IndexedDB test で別途証明済みなので、このスライスでは DOM 層のみ確認）を使い、受信箱一覧・スレッド表示・In-Reply-To によるスレッド併合を目視確認した。compose/search/command palette/account設定/prerotation/custom-domain/`.eml` import/push・SSE は明示的に対象外（下記の未実装項目が該当）。
...
| 2026-08-24 | 作業中 | PLAN.md §3.2「duplicate event / replay の validation」を実装。`commitLocalMutation`（client 自身の mutation 書き込み経路）が `commitIngress` と同じ `vault_events` 実 unique keyPath で dedup されることを実 IndexedDB で確認（`test/protocol/vault-store-durability.test.ts`）。調査の過程で `VaultEventV1.parents` が署名対象に含まれるだけで実際には一切使われていない（causal ordering にも存在チェックにも使わない vestigial field）ことを発見し、gap として明記した |

| 2026-08-24 | 作業中 | PLAN.md §3.2「MLS/device signer 接続」を実証。`VaultEventSigner`/`SegmentKeyWrapSigner` の構造的同一性により `buildVaultCryptoBoundary` の実 MLS signer は特別な接続コード無しで `VaultBackedLocalJmapMutationSink` にそのまま使えることを、実 MLS self group を通した end-to-end test（`test/protocol/identity-local-jmap-write-path.test.ts`）で確認——`Email/set` の署名が別インスタンスの実 MLS verifier（`buildVaultDeliveryProjector`）で受理・projection まで通ることを検証した |

| 2026-08-24 | 作業中 | PLAN.md のドキュメント精度を監査・修正。§4.1「`ClientState` から作る `MlsSelfGroupProvider` 具象実装」は `mls/store.ts` の `StoredMlsSelfGroupProvider` として既に完成済みだった（チェックリストが古いまま）ため `[x]` に訂正。§4.3「projection rebuild」は §5.2 の `rebuildLocalJmapProjection` が record-source-agnostic なので実は既に使えたが、restore-transfer の commit 結果に対して実際に確認した test が無かったため `test/protocol/identity-restore-transfer-projection-rebuild.test.ts` を追加（実 MLS self group + 実 restore-transfer round trip + `buildLocalJmapProjectionRebuild` で正しい projection を再構築することを確認）。recovery archive import 側も同じ store を使うため理屈上は動くはずだが、専用の確認 test はまだ無いと明記した |
...
| 2026-08-24 | `2f78321`〜`e348da8` | PLAN.md §7「read-only inbox first slice」を実装（ユーザーと協議のうえ「まず受信箱一覧＋閲覧だけ」にスコープを絞る計画を承認）。`buildLocalJmapReadModel`（`2f78321`）→ `src/mail/message-view.ts`/`body-text.ts`（`1ea0eeb`、threading.ts の union-find 移植＋新規 body/header 抽出）→ `src/ui/thread.ts`/`left-pane.ts`/`shell.ts`/`format.ts`（`7124ef0`、`thread.ts` のレンダリングロジック移植、`left-pane.ts` は `InboxSummary` ではなく `ThreadGroup` 単位に作り直し）→ `index.html`/`style.css`/`main.ts` 配線（`e348da8`）の順で実装。`bun run build` + `dist/index.html` を file:// で開き、mock read model で受信箱一覧・スレッド表示・In-Reply-To によるスレッド併合を目視確認（データ層は既存の実 MLS test で別途証明済み）。compose/search/account設定等は明示的に未着手のまま §7 に残す |
| 2026-08-24 | `6e9700b`〜`8568c76` | PLAN.md §6.2「SMTP listener」を実装（ユーザーと協議のうえ resolveRecipient は subdomain-per-identity did:webvh のライブ解決で確定、SMTP listener スコープは実際に:25 で listen する net.Server まで確定、TTL default は vault delivery と揃えて30日に確定）。`mail-recipient-resolver.ts`/`mail-smtp-protocol.ts`（純粋な状態機械、`jmapsmtp.bak`のRust実装から protocol shape を移植しつつ unknown recipient は 550 で拒否するよう反転——理由はコード内コメント参照、`6e9700b`）→ `mail-smtp-listener.ts`（実 socket・STARTTLS、`8568c76`）→ `core/index.ts` 配線（`8568c76`）の順で実装。実装中に server-side `socket.upgradeTLS({isServer:true})` が Bun 1.4 の新規APIで手元の 1.3.14 に無いと判明（`openssl s_client -starttls smtp` で実ハンドシェイクを確認しながら特定）——ユーザーと協議し Bun/@types/bun を 1.4.0 へアップグレード。48 unit test + 5 real-socket integration test（STARTTLS込み）で確認、`bun test test/protocol/ test/core/` 372件全通過でリグレッション無し確認 |
| 2026-08-24 | `cf47ced` | PLAN.md §6.2「outbound SMTP client」を実装。調査でPGP暗号化が相手PGP公開鍵取得手段（旧実装はrelayの`/pgp/peerkey`キャッシュ依存、relayは廃止予定）を欠く——DIDCommのwire format同様の別途設計が必要なギャップと判明、ユーザーと協議し平文送信を先に進める方針を確定。さらにoutbound送信全体も「device→core向け認証付きnarrow-API」「vault側outbox管理」「outbound SMTP client本体」に分解でき、後2つはvault/protocol変更不要（`transport.result` kindが既存予約済み・未使用のまま流用できると判明）と分かったため、まずclient本体のみを実装する方針をユーザーと協議し確定。`mail-smtp-client.ts`（`jmapsmtp.bak`のsmtp_out.rsから移植、MX解決・client STARTTLS・RFC 5321 dialogue、`cf47ced`）を実装済みのinbound listenerへ実配送するテストで相互検証。narrow-API/vault配線/EmailSubmission set/PGPは明示的に次の作業として残す |
| 2026-08-24 | `686e373`〜`50b7888` | PLAN.md §6.2「outbound送信の残り（narrow-API + vault配線）」を実装、outbound送信を完了。既存の ingress/vault-delivery 等と全く同じ形（`protocol/*.ts` request 型 + signing bytes + `DeviceControlSignatureVerifier` + `rosterBacked*Authorizer` + HTTP handler + `Core*Transport`）を mail submission にもう一つ追加するだけで済むと判明（`686e373` protocol層、`a518562` core層）。vault側は `VaultBackedLocalJmapMutationSink.emailSet` の汎用 commit sequence を `commitIntents` として切り出し再利用、reducer に `transport.result` no-op を1行追加するだけで新しい vault event kind 不要と確定（`f64dafc`）。`identity/bootstrap.ts` に `buildMailSubmitter`（mailFrom は identity 自身の DID から派生、新フィールド不要）を追加し、最小 `EmailSubmission/set`（単一 create のみ）も同時配線。capstone e2e test（`50b7888`）で実 vault→narrow-API→outbound SMTP→inbound listener→受信側 pullable ingress を通しで実証、sender 側 projection rebuild で outbox→sent 遷移も確認。`bun test test/protocol/ test/core/` 396件全通過 |
